...
export default function BookingModal({
  ...
  const [step, setStep] = useState<'details' | 'guests' | 'confirmed'>('details');
  ...
  const [guestEmail, setGuestEmail] = useState('sushmita@example.com');
  ...
  const handleCompleteReservation = (e: React.FormEvent) => {
    ...
    const randomRef = 'DT-' + Math.floor(10000 + Math.random() * 90000);
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100 bg-stone-50/70">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-stone-500">{restaurant.neighborhood} • {restaurant.cuisine}</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {step === 'details' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 text-xs font-semibold text-amber-700 bg-amber-50 px-3 py-1.5 rounded-xl">
                ...
                <span>Step 1 of 2: Select Dining Preferences & Time Slot</span>
              </div>

              {/* 1. Guests Party Size Selector */}
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  {['Today, Sep 4', 'Tomorrow, Sep 5', 'Saturday, Sep 6', 'Sunday, Sep 7'].map((d) => (
                    <button
                      key={d}
                      type="button"
                      onClick={() => setDate(d)}
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 uppercase tracking-wider mb-2 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-amber-700 font-medium">Free instant confirmation</span>
                </label>
                    ...
                    <label
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl border text-xs cursor-pointer transition-colors ${
                        seatingArea === area.name
                          ? 'bg-amber-50/70 border-amber-500 text-stone-900 font-semibold'
                          : 'bg-stone-50/60 border-stone-200 hover:bg-stone-100 text-stone-700'
                      }`}
                    >
                      <div>
                        <div className="font-bold">{area.name}</div>
                        <div className="text-[11px] text-stone-500">{area.desc}</div>
                      </div>
                      <input
                        type="radio"
                        name="seating"
              ...
              <div className="pt-3 border-t border-stone-100 flex items-center justify-end">
                <button
                  ...
                  className="bg-amber-600 hover:bg-amber-500 text-white px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-amber-600/25 transition-all cursor-pointer"
                >
          ...
          {step === 'guests' && (
            <form onSubmit={handleCompleteReservation} className="space-y-5">
              <div className="flex items-center justify-between bg-amber-50 px-3 py-2 rounded-xl text-xs font-semibold text-amber-900">
                ...
                <span className="text-[11px] bg-amber-200/70 text-amber-900 px-2 py-0.5 rounded-md">
                  {seatingArea}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-stone-700 mb-1">Occasion</label>
                  <select
                    value={occasion}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                    placeholder="e.g. Dietary allergies, high chair, flowers"
                  />
                </div>
              </div>
              ...
              <div className="bg-stone-50 rounded-xl p-3.5 border border-stone-200 text-xs text-stone-600 flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-stone-900">DineTable Guarantee & Policy</div>
                  <div className="mt-0.5 text-[11px] text-stone-500">
                    Free cancellation up to 2 hours before dining time. No credit card required to hold standard tables.
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-stone-100 flex items-center justify-between">
                ...
                <button
                  type="submit"
                  className="bg-amber-600 hover:bg-amber-500 text-white px-7 py-3 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-amber-600/25 transition-all cursor-pointer active:scale-95"
                >
            ...
            <div className="py-6 text-center space-y-5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-stone-500">
                  Confirmation and digital pass sent to{' '}
                  <span className="font-semibold text-stone-800">{confirmedReservation.guestEmail}</span>
                </p>
              </div>
              ...
              <div className="max-w-md mx-auto bg-stone-50 border border-stone-200 rounded-2xl p-4 text-left space-y-3">
                <div className="flex items-center justify-between border-b border-stone-200 pb-2">
                  <div>
                    <div className="text-[10px] text-stone-400 font-bold uppercase">Booking Reference</div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-stone-400 block text-[10px]">Restaurant</span>
                    <span className="font-bold text-stone-900">{confirmedReservation.restaurantName}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-stone-400 block text-[10px]">Seating Area</span>
                    <span className="font-bold text-stone-900">{confirmedReservation.seatingArea}</span>
                  </div>
                </div>

                <div className="pt-2 border-t border-stone-200 text-[11px] text-stone-500 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                  <span>{confirmedReservation.restaurantAddress}</span>
                </div>
              </div>
